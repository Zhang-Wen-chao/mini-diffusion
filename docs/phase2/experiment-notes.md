...
### 发现 3：flow 少步优势待验证（60k 数据补齐后下结论）
- 30k 时 flow 1 步 701 → 50 步 178：单调改善，符合"路径不直→需要步数"。
- 但 flow 50 步 (178) > ddpm 50 步 (94.6)：同训练量下 ddpm 反超。
- 期待 60k 后：flow 的**少步优势**（1-4 步 vs ddpm 10 步）是它存在的意义，
  50 步以上对比则 ddpm/flow 应接近。
