"""进程组构建（移植自 mini-megatron）：rank 布局 TP → PP → DP

布局规则：
  rank = tp_rank + tp_size * (pp_rank + pp_size * dp_rank)
  TP 组：连续 tp_size 个 rank
  PP 组：间隔 tp_size
  DP 组：间隔 tp_size*pp_size
"""
...
def init_model_parallel(tp_size=1, pp_size=1):
    ...
    return {
        ...
        "tp_group": tp_group, "pp_group": pp_group, "dp_group": dp_group,
    }
